...
piewall manages Windows Defender Firewall rules: Python 3.12, uv, Tkinter GUI, argparse CLI,
MCP server.
...
- Layout: `src/piewall/` (`model`, `conflicts`, `transfer` are pure and OS-independent;
  `backend`/`elevate`/`listeners` are Windows-specific; `cli`, `gui`, `theme`, `mcp_server`).
...
- Packaging: `packaging/` (PyInstaller + Inno Setup), site: `site/` (Astro → Cloudflare),
  brand: `brand/`.
